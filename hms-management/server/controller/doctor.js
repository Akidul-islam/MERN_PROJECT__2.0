...
const getDoctors = async (req, res, next) =>{
   ...
   try {
      ...
      res.status(200).json({ status:'succes', data:{ doctors, isbn: doctors.length }})
...
const patchDoctorUpdate = async (req, res, next) => {
   const {doctorId} = req.params
 try {
   const doctor = await serviceDc.doctorUpdateService(doctorId, req.body, req.user)
   res.status(203).json({ status:'success', doctor})
 } catch (error) {
   next(error)
 }
}
...
const deletedDoctor =  async (req, res, next) =>{
   ...
   try { 
      ...
      res.status(200).json({ results:`doctor ${doctorId} successfully deleted`})
...
module.exports= {getDoctor, getDoctors, createDoctor, patchDoctorUpdate, deletedDoctor}