...
const registerService = async ({ name, email, password, role }) => {
  ...
  const userCreate = await db.createNewUser({
    name,
    email,
    password: hash,
    role,
  ...
  if (userCreate.role.includes('patient')) {
    await crudOperation.dbCreateNewItem(PatientProfile, {
      userId: userCreate._id,
    });
  }
  if (userCreate.role.includes('doctor')) {
    await crudOperation.dbCreateNewItem(DoctorProfile, {
      userId: userCreate._id,
    });
  }
  if (userCreate.role.includes('admin')) {
    await crudOperation.dbCreateNewItem(AdminProfile, {
      userId: userCreate._id,
    });
  }
  return userCreate;
